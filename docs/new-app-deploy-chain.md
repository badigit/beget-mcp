...
жёстко называет одну из них. Домен и его воркер поэтому могут разъехаться по версиям
незаметно — на живой платформе так и есть: домен на 8.5, а один из воркеров запускается
через `php8.4`. Меняя PHP домену, проверь `cron_list`.
